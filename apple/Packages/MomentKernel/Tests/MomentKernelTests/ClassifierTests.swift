...
struct ClassifierTests {
    ...
    @Test func 판정_단위가_인접쌍이_아니라_군집이다() {
        // 인접쌍 임계값이면 0→15→30→45가 전부 한 덩어리가 된다(각 간격 15 < 20).
        // 평균 연결 군집은 양 끝이 45 떨어진 것을 본다.
        let result = Classifier.classify([
            photo("2026080213000000", position: (0, 0, 0)),
            photo("2026080213010000", position: (15, 0, 0)),
            photo("2026080213020000", position: (30, 0, 0)),
            photo("2026080213030000", position: (45, 0, 0)),
        ])
