...
struct ClassifierTests {
    ...
    @Test func 판정_단위가_인접쌍이_아니라_군집이다() {
        // 인접쌍 임계값이면 0→45→90→135가 전부 한 덩어리가 된다(각 간격 45 < 60).
        // 평균 연결 군집은 양 끝이 135 떨어진 것을 본다.
        //
        // **좌표가 임계값을 따라다닌다** — 간격은 늘 임계값의 0.75배다. 기준 A가 임계값을
        // 20에서 60으로 올렸을 때(2026-08-04) 이 픽스처가 유일하게 같이 움직인 자리였다.
        let result = Classifier.classify([
            photo("2026080213000000", position: (0, 0, 0)),
