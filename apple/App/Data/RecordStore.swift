...
/// 디스크에 적히는 형태.
///
/// **도메인 타입과 가른 이유 둘** — ① 커널의 `WallClock`에 저장 형식을 얹지 않는다. 그 타입의
/// 공개 API는 다른 언어 구현이 재현할 계약이고(ADR `0002`) JSON 모양은 이쪽 사정이다
/// ② 스키마 번호는 화면이 다루는 값이 아니다.
...
/// 저장용 16자리 표현. 촬영 파일명과 같은 자릿수 배열이라 눈으로 읽힌다.
///
