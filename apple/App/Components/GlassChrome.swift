...
/// ⚠️ **iPad 지면에서는 가림막을 안 깐다** (사용자 판정 2026-08-12) — 뒤에 지나갈 것을
/// 가려 버리면 유리가 할 일이 없어진다.
///
/// 필요한 이유는 iPhone에만 있다 — 유리 캡슐 위쪽의 **상태 표시줄**이 비어서, 밝은 사진이
/// 지나가면 시각·배터리가 안 읽힌다. iPad 지면은 상태 표시줄 아래에 있다.
